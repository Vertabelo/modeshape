 ...
 */
package org.modeshape.sequencer.ddl.dialect.postgres_10plus;
 ...
 *
 */
public class Postgres10PlusDdlParserTest extends DdlParserTestHelper {
    private static final String SPACE = DdlConstants.SPACE;

    public static final String DDL_FILE_PATH = "ddl/dialect/postgres_10plus/";

    @Before
    public void beforeEach() {
        parser = new Postgres10PlusDdlParser();
    ...
    @Test
    public void shouldParseCreateSequenceWithDataType() {
        printTest("shouldParseCreateSequenceWithDataType()");
        // as
        String content = "CREATE SEQUENCE serial START 101 AS smallint;";
        assertScoreAndParse(content, null, 1);
        AstNode childNode = rootNode.getChildren().get(0);
        assertTrue(hasMixinType(childNode, TYPE_CREATE_SEQUENCE_STATEMENT));

        assertEquals("smallint", childNode.getProperty(Postgres10PlusDdlLexicon.SEQ_DATA_TYPE));
    }
    ...
    public void shouldParseCreateTable_6() {
        ...
        // partition by
        final String filename = "postgres_10plus_tableModern1.ddl";
        String content = getFileContent(DDL_FILE_PATH + filename);
        assertScoreAndParse(content, DDL_FILE_PATH + filename, 1);
        AstNode tableNode = rootNode.getChildren().get(0);
        assertTrue(hasMixinType(tableNode, TYPE_CREATE_TABLE_STATEMENT));

        assertEquals("RANGE (logdate)", tableNode.getProperty(Postgres10PlusDdlLexicon.PARTITION_BY));
    }

    @Test
    public void shouldParseCreateTable_7() {
        printTest("shouldParseCreateTable_7()");
        // partition of, partition bound
        final String filename = "postgres_10plus_tableModern2.ddl";
        String content = getFileContent(DDL_FILE_PATH + filename);
        assertScoreAndParse(content, DDL_FILE_PATH + filename, 1);
        AstNode tableNode = rootNode.getChildren().get(0);
        assertTrue(hasMixinType(tableNode, TYPE_CREATE_TABLE_STATEMENT));

        assertEquals("measurement (unitsales DEFAULT 0)", tableNode.getProperty(Postgres10PlusDdlLexicon.PARTITION_OF));
        assertEquals("FROM ('2016-07-01') TO ('2016-08-01')", tableNode.getProperty(Postgres10PlusDdlLexicon.PARTITION_BOUND));
    }
    ...
    public void shouldParseCreateTable_8() {
        ...
        // generated
        final String filename = "postgres_10plus_tableModern3.ddl";
        String content = getFileContent(DDL_FILE_PATH + filename);
        assertScoreAndParse(content, DDL_FILE_PATH + filename, 1);
        AstNode tableNode = rootNode.getChildren().get(0);
        assertTrue(hasMixinType(tableNode, TYPE_CREATE_TABLE_STATEMENT));
        ...
        AstNode columnNode = tableNode.getChildren().get(0);
        assertTrue(hasMixinType(columnNode, TYPE_COLUMN_DEFINITION));
        assertEquals("ALWAYS", columnNode.getProperty(Postgres10PlusDdlLexicon.COLUMN_IDENTITY));
    }
    ...
    public void shouldParseCreateTable_9() {
        ...
        final String filename = "postgres_10plus_tableModern4.ddl";
        String content = getFileContent(DDL_FILE_PATH + filename);
        assertScoreAndParse(content, DDL_FILE_PATH + filename, 1);
        AstNode tableNode = rootNode.getChildren().get(0);
        assertTrue(hasMixinType(tableNode, TYPE_CREATE_TABLE_STATEMENT));
        ...
        AstNode columnNode = tableNode.getChildren().get(0);
        assertTrue(hasMixinType(columnNode, TYPE_COLUMN_DEFINITION));
        assertEquals("BY DEFAULT", columnNode.getProperty(Postgres10PlusDdlLexicon.COLUMN_IDENTITY));
        assertEquals("START WITH 10 INCREMENT BY 10", columnNode.getProperty(Postgres10PlusDdlLexicon.COLUMN_IDENTITY_OPTIONS));
    }
