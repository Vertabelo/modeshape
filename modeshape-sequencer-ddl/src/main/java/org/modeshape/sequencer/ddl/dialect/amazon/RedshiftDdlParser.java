    ...
    private void parseIndexColumnExpressionList( final String columnExpressionList,
        ...
        if (!tokens.matches(R_PAREN)) {
            ...
            if (!possibleColumns.isEmpty()) {
                ...
                final boolean tableIndex = indexNode.hasMixin(RedshiftDdlLexicon.TYPE_CREATE_INDEX_STATEMENT);

                // find appropriate table nodes
                if (tableIndex) {
                    // table index so find table node
                    final String tableName = (String)indexNode.getProperty(RedshiftDdlLexicon.TABLE_NAME);
                    ...
                    if (!nodes.isEmpty()) {
                        if (nodes.size() == 1) {
                            ...
                            for (final AstNode node : nodes) {
                                if (node.hasMixin(TYPE_CREATE_TABLE_STATEMENT)) {
                    ...
                    // must be bitmap-join
                    tableNodes = indexNode.getChildren(RedshiftDdlLexicon.TYPE_TABLE_REFERENCE);
                }
                    ...
                    for (String possibleColumn : possibleColumns) {
                        ...
                        if (tableIndex) {
                            if (tableNodes.isEmpty()) {
                                ...
                                final List<AstNode> columnNodes = tableNode.getChildren(TYPE_COLUMN_DEFINITION);
                                ...
                                if (!columnNodes.isEmpty()) {
                                    ...
                                    for (final AstNode colNode : columnNodes) {
                                        if (colNode.getName().toUpperCase().equals(possibleColumn.toUpperCase())) {
                                            ...
                                            if (asc || desc) {
                                                colRef.addMixin(RedshiftDdlLexicon.TYPE_INDEX_ORDERABLE);

                                                if (asc) {
                                                    colRef.setProperty(RedshiftDdlLexicon.INDEX_ORDER, "ASC");
                                                } else {
                                                    colRef.setProperty(RedshiftDdlLexicon.INDEX_ORDER, "DESC");
                                                }
                                            }
            ...
            if (!functions.isEmpty()) {
                indexNode.setProperty(RedshiftDdlLexicon.OTHER_REFS, functions);
            }
        }
