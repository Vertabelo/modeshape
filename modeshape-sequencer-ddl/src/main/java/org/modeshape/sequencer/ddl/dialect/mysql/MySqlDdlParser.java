...
import static org.modeshape.sequencer.ddl.dialect.mysql.MySqlDdlLexicon.COLUMN_FORMAT;
import static org.modeshape.sequencer.ddl.dialect.mysql.MySqlDdlLexicon.COLUMN_STORAGE;
import static org.modeshape.sequencer.ddl.dialect.mysql.MySqlDdlLexicon.COMMENT;
import static org.modeshape.sequencer.ddl.dialect.mysql.MySqlDdlLexicon.TABLE_LIKE;
    ...
    */
    ...
    protected AstNode parseCreateStatement( DdlTokenStream tokens,
        ...
        if (tokens.matches(STMT_CREATE_INDEX)
                || tokens.matches(MySqlDdlConstants.MySqlStatementStartPhrases.STMT_CREATE_ONLINE_INDEX)
                || tokens.matches(MySqlDdlConstants.MySqlStatementStartPhrases.STMT_CREATE_OFFLINE_INDEX)
                || tokens.matches(MySqlDdlConstants.MySqlStatementStartPhrases.STMT_CREATE_UNIQUE_INDEX)
                || tokens.matches(MySqlDdlConstants.MySqlStatementStartPhrases.STMT_CREATE_FULLTEXT_INDEX)
                || tokens.matches(MySqlDdlConstants.MySqlStatementStartPhrases.STMT_CREATE_SPATIAL_INDEX)
                || tokens.matches(MySqlDdlConstants.MySqlStatementStartPhrases.STMT_CREATE_ONLINE_UNIQUE_INDEX)
                || tokens.matches(MySqlDdlConstants.MySqlStatementStartPhrases.STMT_CREATE_ONLINE_FULLTEXT_INDEX)
                || tokens.matches(MySqlDdlConstants.MySqlStatementStartPhrases.STMT_CREATE_ONLINE_SPATIAL_INDEX)
                || tokens.matches(MySqlDdlConstants.MySqlStatementStartPhrases.STMT_CREATE_OFFLINE_UNIQUE_INDEX)
                || tokens.matches(MySqlDdlConstants.MySqlStatementStartPhrases.STMT_CREATE_OFFLINE_FULLTEXT_INDEX)
                || tokens.matches(MySqlDdlConstants.MySqlStatementStartPhrases.STMT_CREATE_OFFLINE_SPATIAL_INDEX)) {
            ...
            return parseStatement(tokens,
                    ...
                    parentNode,
                    MySqlDdlLexicon.TYPE_CREATE_DATABASE_STATEMENT);
        }
    ...
    private AstNode parseCreateIndexStatement(DdlTokenStream tokens, AstNode parentNode) {
        ...
        if (online != null) {
            indexNode.setProperty(MySqlDdlLexicon.ONLINE, online);
        }
        if (type != null) {
            indexNode.setProperty(MySqlDdlLexicon.TYPE, type);
        }
        ...
        indexNode.setProperty(MySqlDdlLexicon.TABLE_NAME, tableName);
...
//        140120;140101;1;'Tablespace';null;5
        ...
        while (tokens.hasNext() && !isTerminator(tokens) && !tokens.matches("PARTITION") && lastMatched) {
    ...
    private boolean parseTableOption(DdlTokenStream tokens, AstNode tableNode) {
        ...
        if (tokens.canConsume("ENGINE")) {
            ...
            tableNode.setProperty(MySqlDdlLexicon.TABLE_ENGINE, engineName);
            ...
            tableNode.setProperty(MySqlDdlLexicon.TABLE_CHARACTER_SET, charsetName);
            
        } else if (tokens.canConsume("DEFAULT", "COLLATE")) {
            tokens.canConsume("=");
            String collationName = parseName(tokens);

            tableNode.setProperty(MySqlDdlLexicon.TABLE_COLLATE, collationName);
            ...
            tableNode.setProperty(MySqlDdlLexicon.COMMENT, comment);
        } else if (tokens.canConsume("TABLESPACE")) {
            tokens.canConsume("=");
            String tablespaceName = parseName(tokens);

            tableNode.setProperty(MySqlDdlLexicon.TABLE_TABLESPACE, tablespaceName);
        } else {
...
//    | CHECK (expr)
    ...
    private void parseIndexColumn(DdlTokenStream tokens, AstNode parentNode, String referenceType) {
        ...
        if (tokens.canConsume("ASC")) {
            indexColumnNode.setProperty(MySqlDdlLexicon.INDEX_ORDER, "ASC");
        } else if (tokens.canConsume("DESC")) {
            indexColumnNode.setProperty(MySqlDdlLexicon.INDEX_ORDER, "DESC");
        }
    }
...
//    | CHECK (expr)
        ...
        if (tokens.matches("FULLTEXT", "INDEX")
            ...
            indexNode.setProperty(MySqlDdlLexicon.TABLE_NAME, tableNode.getName());
            indexNode.setProperty(MySqlDdlLexicon.TYPE, fullText);
    ...
    private void parseIndexType(DdlTokenStream tokens, AstNode indexNode) {
        ...
        if (tokens.canConsume("USING")) {
            if (tokens.canConsume("BTREE")) {
                indexNode.setProperty(MySqlDdlLexicon.INDEX_TYPE, "BTREE");
            } else {
                tokens.consume("HASH");
                indexNode.setProperty(MySqlDdlLexicon.INDEX_TYPE, "HASH");
            }
        }
...
//          | COMMENT 'string'
        if (tokens.canConsume("KEY_BLOCK_SIZE")) {
            ...
            indexNode.setProperty(MySqlDdlLexicon.KEY_BLOCK_SIZE, keyBlockSize);
        } else if (tokens.matches("USING")) {
            parseIndexType(tokens, indexNode);
        } else if (tokens.canConsume("WITH", "PARSER")) {
            String parserName = tokens.consume();
            indexNode.setProperty(MySqlDdlLexicon.WITH_PARSER, parserName);
        } else if (tokens.canConsume("COMMENT")) {
            String comment = tokens.consume();
            indexNode.setProperty(MySqlDdlLexicon.COMMENT, comment);
        }
...
//        [ON UPDATE reference_option]
    ...
    protected void parseReferences( DdlTokenStream tokens,
        ...
        if (tokens.canConsume("MATCH", "FULL")) {
            constraintNode.setProperty(MySqlDdlLexicon.MATCH, "FULL");
        } else if (tokens.canConsume("MATCH", "PARTIAL")) {
            constraintNode.setProperty(MySqlDdlLexicon.MATCH, "PARTIAL");
        } else if (tokens.canConsume("MATCH", "SIMPLE")) {
            constraintNode.setProperty(MySqlDdlLexicon.MATCH, "SIMPLE");
        }
    ...
    protected String getReferentialActionType(DdlTokenStream tokens) {
        if (tokens.canConsume("SET NULL")) {
            return MySqlDdlLexicon.SET_NULL_ACTION;
        }
        if(tokens.canConsume("NO", "ACTION")) {
            return MySqlDdlLexicon.NO_ACTION;
        } 
    ...
    private boolean parseAutoIncrementClause(DdlTokenStream tokens, AstNode columnNode) {
        if (tokens.canConsume("AUTO_INCREMENT")) {
            columnNode.setProperty(MySqlDdlLexicon.COLUMN_AUTO_INCREMENT, true);
            return true;
        }
        return false;
    }
            ...
            if (tokens.canConsume("ADD")) {
                ...
                if (isTableConstraint(tokens)) {
                    ...
                    if (tokens.matches(L_PAREN)) {
                        ...
                        DdlTokenStream columnDefinitionsTokens = new DdlTokenStream(content, DdlTokenStream.ddlTokenizer(true), false);
                ...
                if (tokens.canConsume("PRIMARY", "KEY")) {
                    nodeFactory().node("DROP_PRIMARY_KEY", alterTableNode, MySqlDdlLexicon.TYPE_DROP_PRIMARY_KEY);
                } else if (tokens.canConsume("INDEX") || tokens.canConsume("KEY")) {
                    String indexName = parseName(tokens);
                    nodeFactory().node(indexName, alterTableNode, MySqlDdlLexicon.TYPE_DROP_INDEX);
                } else if (tokens.canConsume("FOREIGN", "KEY")) {
                    String fkName = parseName(tokens);
                    nodeFactory().node(fkName, alterTableNode, MySqlDdlLexicon.TYPE_DROP_FOREIGN_KEY);
                } else if (tokens.canConsume("PARTITION")) {
                    parseUntilCommaOrTerminator(tokens);
                } else {
                    tokens.canConsume("COLUMN");
                ...
                if (columnNodes.size() > 0) {
                    ...
                    nodeFactory().setType(columnNode, MySqlDdlLexicon.TYPE_CHANGE_COLUMN_DEFINITION);
                    columnNode.setProperty(MySqlDdlLexicon.OLD_NAME, oldName);
                    parseAlterColumnFirstOrAfterClause(tokens, columnNode);
                }
                ...
                if (columnNodes.size() > 0) {
                    ...
                    nodeFactory().setType(columnNode, MySqlDdlLexicon.TYPE_MODIFY_COLUMN_DEFINITION);
                    parseAlterColumnFirstOrAfterClause(tokens, columnNode);
                }
                ...
                nodeFactory().node(newTableName, alterTableNode, MySqlDdlLexicon.TYPE_RENAME_TABLE_STATEMENT);
            } else {
    ...
    private void parseAlterColumnFirstOrAfterClause(DdlTokenStream tokens, AstNode columnNode) {
        if (tokens.canConsume("FIRST")) {
            columnNode.setProperty(MySqlDdlLexicon.FIRST, true);
        } else if (tokens.canConsume("AFTER")) {
            String colName = parseName(tokens);
            columnNode.setProperty(MySqlDdlLexicon.AFTER, colName);
        }
    }
...
//        [WITH [CASCADED | LOCAL] CHECK OPTION]
        ...
        if (! tokens.canConsume("VIEW")) {
            return parseStatement(tokens,
                    ...
                    parentNode,
                    MySqlDdlLexicon.TYPE_CREATE_SQL_SECURITY_STATEMENT);
        }
...
//        [WITH [CASCADED | LOCAL] CHECK OPTION]
        ...
        if (! tokens.canConsume("VIEW")) {
            return parseStatement(tokens,
                    ...
                    parentNode,
                    MySqlDdlLexicon.TYPE_CREATE_ALGORITHM_STATEMENT);
        }
...
//        [WITH [CASCADED | LOCAL] CHECK OPTION]
    ...
    protected String parseUntilTerminatorOrWithCheck( DdlTokenStream tokens ) throws ParsingException {
        ...
        while (tokens.hasNext() && !tokens.matches(DdlTokenizer.STATEMENT_KEY) && ((doUseTerminator() && !isTerminator(tokens)) || !doUseTerminator())
                && !(tokens.matches("WITH", "CHECK", "OPTION") || tokens.matches("WITH", DdlTokenStream.ANY_VALUE, "CHECK", "OPTION")) ) {
    ...
    class MySqlDataTypeParser extends DataTypeParser implements MySqlDdlConstants.MySqlDataTypes {
        ...
        protected DataType parseCharStringType( DdlTokenStream tokens ) throws ParsingException {
            ...
            MySqlDataType result = new MySqlDataType(superResult.getName());
        ...
        protected DataType parseCustomType( DdlTokenStream tokens ) throws ParsingException {
            ...
            if (tokens.matches(DTYPE_FIXED) || tokens.matches(DTYPE_DOUBLE)) {
                ...
                dataType = new MySqlDataType(typeName);
                if (tokens.canConsume("COLLATE")) {
                    String collate = tokens.consume();
                    ((MySqlDataType)dataType).setCollate(collate);
                }
                if (tokens.canConsume("CHARACTER", "SET")) {
                    String charset = tokens.consume();
                    ((MySqlDataType)dataType).setCharacterSet(charset);
                }
                if (tokens.canConsume("COLLATE")) {
                    String collate = tokens.consume();
                    ((MySqlDataType)dataType).setCollate(collate);
                ...
                dataType = new MySqlDataType(typeName + values);
                if (tokens.canConsume("COLLATE")) {
                    String collate = tokens.consume();
                    ((MySqlDataType)dataType).setCollate(collate);
                }
        ...
        public void setPropertiesOnNode( AstNode columnNode,
            ...
            if (datatype instanceof MySqlDataType) {
                String characterSet = ((MySqlDataType) datatype).getCharacterSet();
                if (characterSet != null) {
                    columnNode.setProperty(MySqlDdlLexicon.TABLE_CHARACTER_SET, characterSet);
                }
                String collate = ((MySqlDataType) datatype).getCollate();
                if (collate != null) {
                    columnNode.setProperty(MySqlDdlLexicon.TABLE_COLLATE, collate);
                }
                
                boolean onUpdateCurrentTimestamp = ((MySqlDataType) datatype).isOnUpdateCurrentTimestamp();
                if (onUpdateCurrentTimestamp) {
                    columnNode.setProperty(MySqlDdlLexicon.ON_UPDATE_CURRENT_TIMESTAMP, true);
                }
            }
        }
    ...
    protected DdlTokenizer ddlTokenizer(boolean includeComments) {
        return new MySqlDdlTokenizer(includeComments);
    }
